...
import { Button, Divider, Flex, Grid, SliderField, TextField } from "@aws-amplify/ui-react";
...
export default function MyForm(props) {
    ...
    return (<Grid as="form" rowGap="15px" columnGap="15px" padding="20px" onSubmit={async (event) => {
        ...
        }} {...getOverrideProps(overrides, "MyForm")} {...rest}><TextField label="email" value={Field0} onChange={e => {
            let { value } = e.target;
            if (onChange) {
                const modelFields = {
                    Field0: value,
            ...
            setField0(value);
        }} onBlur={() => runValidationTasks("Field0", Field0)} errorMessage={errors.Field0?.errorMessage} hasError={errors.Field0?.hasError} {...getOverrideProps(overrides, "Field0")}></TextField><SliderField label="slide" value={Field1} onChange={e => {
            let value = e;
            if (onChange) {
                const modelFields = {
                    Field0,
            ...
            setField1(value);
        }} onBlur={() => runValidationTasks("Field1", Field1)} errorMessage={errors.Field1?.errorMessage} hasError={errors.Field1?.hasError} {...getOverrideProps(overrides, "Field1")}></SliderField><Divider orientation="horizontal" {...getOverrideProps(overrides, "SectionalElement0")}></Divider><Flex justifyContent="space-between" {...getOverrideProps(overrides, "CTAFlex")}><Button children="Clear" type="reset" onClick={(event) => {
            event.preventDefault();
            resetStateValues();
